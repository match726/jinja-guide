...
const AdminRegisterShrineDetails = () => {
  ...
  // フォームの初期状態を定義
  const [formSections, setFormSections] = useState<FormProps[]>(RegisterShrineDetailsFormProps)
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const options = {
      method: "POST",
      url: BACKEND_ENDPOINT + "/api/admin/regist/shrine-details",
      headers: {
    ...
    // フォームを初期状態に戻す
    setFormSections(RegisterShrineDetailsFormProps);
  ...
  return (
    <>
      ...
      <div className="bg-gradient-to-b from-red-50 to-white flex items-top justify-center p-8">
        <div className="bg-gradient-to-b from-red-50 to-white flex items-top justify-center p-8">
          <RegisterForm title="神社詳細情報登録" sections={formSections} onValueChange={handleValueChange} onAddField={handleAddField} onRemoveField={handleRemoveField} onSubmit={handleSubmit} />
        </div>
      </div>
    </>
  );
